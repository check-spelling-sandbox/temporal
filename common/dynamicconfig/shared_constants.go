...
type DynamicRateLimitingParams struct {
	...
	// RateBackoffStepSize is the amount by which the rate limit multiplier is reduced when backing off.
	// Should be between 0 and 1
	RateBackoffStepSize float64
	// RateIncreaseStepSize is the amount by which the rate limit multiplier is increased when the
	// system is healthy and current rate < max rate. Should be between 0 and 1.
	RateIncreaseStepSize float64
	// RateMultiMin is the minimum to which the rate limit multiplier can be reduced.
	RateMultiMin float64
	// RateMultiMax is the maximum to which the rate limit multiplier can be increased.
