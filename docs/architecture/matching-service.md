...
Matching Service splits Task Queues into partitions to provide higher throughput overall. Default is 4, but there can be more or less. Partition ownership can be reassigned, and partitions' metadata and task backlog can be loaded/unloaded from storage.
